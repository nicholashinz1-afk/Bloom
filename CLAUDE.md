...
- **Pipeline:** client downscales page images to 1568px JPEG (PDFs are rendered to images client-side via pdf.js from cdnjs, already allowed by CSP), then builds in stages. The **guide** stages (`identify` → title/author/summary/vocabulary, `guide` → big ideas/tips/questions) call `api/lantern.js` on Sonnet; these produce original explanatory content and are never copyright-filtered. The **readable pages** are produced by **on-device OCR (Tesseract.js, loaded from jsdelivr, cached after first use)**, NOT by the model. This is deliberate: asking the model to transcribe a copyrighted book verbatim trips Anthropic's copyright output filter on dense pages (`invalid_request_error: Output blocked by content filtering policy`). OCR mechanically extracts the text she already has, on-device, free, with no filter. `transcribePage()` falls back to the server `transcribe` action only if the OCR engine fails to load, so it's never worse than the AI path. Transcription runs sequentially (single CPU-bound OCR worker) while the guide AI call runs alongside; pages can complete out of order, so `pagesDone` is the contiguous ready prefix (`readyPrefix()`) and `pagesFinished` is the true count. The guide prompts and the voice block live server-side; the voice block is the product, keep it verbatim.
...
- **Her highlights and notes:** tapping a sentence in the readable pages highlights it (marker yellow, distinct from amber vocab terms) with an optional note. Sentence-anchored on purpose: indexes are stable because page text never changes after transcription, and tap beats fiddly selection handles on a phone. Noted sentences show a small pencil mark; the guide view lists all highlights and notes with tap-to-jump. Stored per guide in IndexedDB (`notes_<id>`), deleted with the guide.
- **Ask and explain (the anti-frustration path):** the sentence sheet has a one-tap "What does this mean?" that explains the tapped sentence using the whole page as context (`ask` action, text-only, ~700 max tokens). "Even simpler" re-explains more basically; "Keep with my note" saves the explanation onto the highlight. The guide view has a free-form "Ask about this reading." The ask prompt is instructed to never make the reader feel slow. This was built for a user with poor reading comprehension who gets frustrated when things don't click; the explain path must stay one tap with zero typing.
...
- **~~Detailed medication mode doesn't bridge to completion system.~~** FIXED in v3.9.0. `bridgeDetailedMedSlot()` now sets `td.medication_<slot>` = true when all individual meds in that slot are checked off in detailed mode, so `checkAllDone()` and `getCompletionRate()` work correctly.
...
- **AI reflections:** Default to `claude-haiku-4-5-20251001` for daily journal reflections and cognitive reframes (cheap, fast). Use `claude-sonnet-4-6` for weekly insights and monthly reflections (richer, synthesizes more data). Model whitelist in `api/claude.js` prevents abuse. (The prior `claude-sonnet-4-20250514` snapshot reached end-of-life; `claude.js` aliases it to `claude-sonnet-4-6` so stale/cached clients keep working.)
