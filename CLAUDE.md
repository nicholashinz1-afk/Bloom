...
- **Pipeline:** client downscales page images to 1568px JPEG (PDFs are rendered to images client-side via pdf.js from cdnjs, already allowed by CSP), then calls `api/lantern.js` in stages: `identify` (title/author/summary/vocabulary), `guide` (big ideas, reading tips, self-check questions), `transcribe` (one call per page). The guide call and transcription run concurrently, and transcription uses a 3-wide worker pool; pages can complete out of order, so `pagesDone` is the contiguous ready prefix (`readyPrefix()`), not a count. Sonnet vision model for everything; don't downgrade transcription to Haiku, the readable pages are her accessibility text and fidelity is the product. The prompts and the voice block live server-side; the voice block is the product, keep it verbatim.
